...
int main()
{
    ...
    shm = mmap(0, sizeof(shared_memory), PROT_WRITE | PROT_READ, MAP_SHARED, buf_id, 0);
    ...
    // shm->have_writer = 1;  // подъем флага, запрещающего доступ другим
    ...
    while (1) {
        ...
        cb_offer(&shm->buffer, rand() % 11); // число от 0 до 10
        ...
        pid_t pid = getpid();
        printf("Producer %d writes value = %d\n", pid, cb_peek(&shm->buffer));
