...
int main()
{
    ...
    shm->have_reader = 1; // подъем флага, запрещающего доступ другим
    ...
    while (1) {
        ...
        int result = cb_remove(&shm->buffer);
        ...
        pid_t pid = getpid();
        printf("Consumer %d: Reads value = %d, factorial = %d\n", pid, result, f);
